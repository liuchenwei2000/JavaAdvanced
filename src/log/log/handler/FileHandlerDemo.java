...
public class FileHandlerDemo {
	 ...
	 * 这是因为每条消息被转换成LogRecord，它首先由本地的logger对象使用，这个对象将它传递给它自己的处理器。
	 * 此时，LogRecord被传递给父对象，父对象拥有自己的处理器。
	 * 这个过程将持续下去，直到达到根日志记录器为止。
	 * 根日志记录器有一个缺省的ConsoleHandler，因此消息既在显示器上出现，
	 * 也在日志文件中出现(可以通过调用setUseParentHandlers(false)来关闭这个动作)。
	 */
	public static void main(String[] args) throws Exception {
		 ...
		 * 如果是false，则新的日志会完全覆盖旧的内容，默认是false
		 */
		FileHandler txtHandler = new FileHandler(txtFileName, true);
		/*
		 * FileHandler的缺省输出格式是XML 
		 * 如果想改变这种格式，必须给处理器附加上一个不同的Formatter对象
		 * 该文件使用了一个SimpleFormatter对象，以使输出为纯文本格式
		 */
